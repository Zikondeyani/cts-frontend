<template>
  <div class="overflow-x-auto relative">
    ...
    <div
      class="flex justify-between items-center p-4 space-x-4 mb-4"
      ...
      <tbody class="bg-white divide-y divide-gray-200">
        <tr v-for="(row, index) in filteredData" :key="index">
          ...
          <td class="px-6 py-4 whitespace-nowrap">
            {{ row.tonnageAllocation.toFixed(2) }}
          </td>
          <td class="px-6 py-4 whitespace-nowrap">
            {{ row.totalDispatched.toFixed(2) }}
          </td>
          <td class="px-6 py-4 whitespace-nowrap">
  ...
  const filtered = data.filter((item) => {
    return (
      ...
      (!selectedCommodity.value ||
        item.commodity === selectedCommodity.value) &&
...
function exportToExcel() {
  ...
  XLSX.writeFile(workbook, "Commodity_Distribution_Update.xlsx");
}
...
const getCommodities = async () => {
  ...
  commodities.push(
    ...commoditydata.filter((item) => item.commodityType?.Name == "Food")
  );
  return commodities;
};
...
const getDistricts = async () => {
  ...
  districts.length = 0;
  districts.push(
    ...districtsdata.slice().sort((a, b) => a.Name.localeCompare(b.Name))
  );
  return districts;
};
</script>
