...
const exportToExcel = async () => {
 if (
  ...
  !props.dispatchesdataSummary?.length
) {
  Swal.fire({
    toast: true,
    position: 'top-end', // top-right
    icon: 'info',
    title: 'Data is still loading, please try again in a few seconds.',
  ...
  // Helper to ensure numbers are always safe
  const safeNumber = (val) => parseFloat(val || 0) || 0;
  ...
  const dataForExport = filteredData.value.map((item) => ({
    ...
    "Quantity (Mt)": safeNumber(item.loadingPlan?.Quantity).toFixed(2),
    "Total Dispatched (Mt)": item.dispatches
      .reduce((sum, dispatch) => sum + safeNumber(dispatch?.totalDispatched), 0)
      .toFixed(2),
    "Total Received (Mt)": item.dispatches
      .reduce(
        (sum, dispatch) =>
          sum + safeNumber(dispatch?.receiptStats?.totalReceived),
        0
      )
      .toFixed(2),
    "Balance (Mt)": safeNumber(item.loadingPlan?.Balance).toFixed(2),
  ...
  const summaryPerDeliveryNoteData = filteredData.value.flatMap((item) =>
    item.dispatches.flatMap((dispatch) =>
      (dispatch?.receiptStats?.physicalDeliveryNotes || []).map((note) => ({
        "ATC Number": item.loadingPlan?.ATCNumber || "N/A",
        District: item.district || "N/A",
        "Quantity (Mt)": safeNumber(dispatch?.totalDispatched).toFixed(2),
  ...
  const receiptByFDPData = filteredData.value.flatMap((item) =>
    item.dispatches.flatMap((dispatch) =>
      (dispatch.receiptStats?.physicalDeliveryNotes || []).map((note) => ({
        District: item.district || "N/A",
        "ATC Number": item.loadingPlan?.ATCNumber || "N/A",
        "Received (Mt)": safeNumber(
          dispatch.receiptStats?.totalReceived
        ).toFixed(2),
        FDP: note.finalDestination || "N/A",
  ...
  const dispatchedPerTransporterData = filteredData.value.map((item) => ({
    ...
    "ATC Number": item.loadingPlan?.ATCNumber || "N/A",
    Transporter: item.transporter || "N/A",
    "Total Transported (Mt)": item.dispatches
      .reduce((sum, dispatch) => sum + safeNumber(dispatch.totalDispatched), 0)
      .toFixed(2),
  }));
  ...
  const dispatchedPerWarehouseData = filteredData.value
    .map((item) => ({
      ...
      "Tonnage Drawn (Mt)": item.dispatches
        .reduce(
          (sum, dispatch) => sum + safeNumber(dispatch.totalDispatched),
          0
        )
        .toFixed(2),
  ...
  const dispatchesByDateData = props.dispatchesdataSummary.map((item) => ({
    Date: item.dispatchdate
      ? moment(item.dispatchdate).format("YYYY-MM-DD")
      : "N/A",
    District: item.district || "N/A",
    Transporter: item.transporter || "N/A",
    Commodity: item.commodity || "N/A",
    "Total Dispatched (Mt)": safeNumber(item.quantity).toFixed(2),
    FDP: item.fdp || "N/A",
    "ATC #": item.atcnumber || "N/A",
    "Handled By": item.handledby || "N/A",
  }));
