<template>
  <main class="">
    ...
    <div class="max-w-2xl mx-auto px-2 sm:px-6 lg:max-w-5xl lg:px-2">
      ...
      <div class="my-4 border-b border-gray-400">
        <div class="flex flex-wrap">
          <button
            ...
            :class="{
              'text-white': activeTab === 'submitted',
      ...
      <!-- Table -->
      <div
        class="align-middle inline-block min-w-full mt-5 shadow-xl rounded-table bg-white"
      >
        <vue-good-table
          ...
          <template #table-row="props">
            <span v-if="props.column.label == 'Options'">
              ...
              <button
                ...
                class="text-blue-400 hover:text-blue-300 transition duration-300 text-sm"
              >
              ...
              <button
                @click="requestReversal(props.row)"
                v-if="props.row.status !== 4"
                class="text-orange-500 hover:text-orange-700 transition duration-300 ml-2 mr-2 text-sm"
              >
...
const receiptStore = usewarehouseReceiptsStore();
...
const columns = ref([
  ...
  {
  label: "Requirement",
  field: (row) => `${row.totalQuantity} ${row.commodityContainerType}`,
  sortable: true,
  tdClass: "capitalize",
}
,
  ...
  label: "D.Note",
  field: (row) => {
    const notes = [...new Set(row.physicalDeliveryNotes)];
...
const getReceipts = async () => {
  ...
  receiptStore
    ...
    .then((result) => {
      ...
      // Create a map to group receipts by referenceNumber
      const groupedReceipts = {};

      filtered.forEach((entry) => {
        entry.receipts.forEach((receipt) => {
          // Group receipts by referenceNumber
          if (!groupedReceipts[entry.referenceNumber]) {
            groupedReceipts[entry.referenceNumber] = {
              referenceNumber: entry.referenceNumber,
              district: entry.district,
              totalQuantity: 0,
              physicalDeliveryNotes: [],
              remarks: [],
              commodityName: receipt.commodityName, // Assuming all receipts in a group have the same commodity
              commodityType: receipt.commodityType,
              commodityContainerType: receipt.commodityContainerType,
              receipts: []
            };
          }

          // Add receipt data into the grouped object
          groupedReceipts[entry.referenceNumber].totalQuantity += receipt.Quantity;
          groupedReceipts[entry.referenceNumber].physicalDeliveryNotes.push(receipt.PhysicalDeliveryNote);
          groupedReceipts[entry.referenceNumber].remarks.push(receipt.Remarks);
          groupedReceipts[entry.referenceNumber].receipts.push(receipt); // Store the actual receipts
        });
      });

       
      // Flatten grouped data into the receipts array
      receipts.push(...Object.values(groupedReceipts));
