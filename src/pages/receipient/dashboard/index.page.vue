      ...
      <div class="grid grid-cols-1 gap-4 items-start lg:grid-cols-3 lg:gap-8">
        ...
        <div class="grid grid-cols-1 gap-4 lg:col-span-4">
          ...
          <section
            ...
            class="shadow-2xl bg-white rounded-table"
          >
            <div class="container mx-auto p-4">
              <div class="bg-white rounded-lg shadow-md p-4">
                <p class="text-center text-gray-600 mt-4 font-bold mb-2">
                  Recent Dispatches
                </p>

                <div
                  class="align-middle inline-block min-w-full mt-1 rounded-lg"
                >
                  <div class="flex justify-center mb-4">
                    ...
                    >
                      ...
                      <span
                        v-if="leanSeasonCount > 0"
                        class="badge badge-red"
                        >{{ leanSeasonCount }}</span
                      >
                    </button>
                    <button
                      class="tab-button"
                    ...
                    >
                      Emergency Response
                      <span v-if="emergencyCount > 0" class="badge badge-red">{{
                        emergencyCount
                      }}</span>
                    </button>
                  </div>
...
const columns = ref([
...

  {
    ...
    hidden: false,
    field: (row) =>
      `<span >HandledBy: ${row.loadingPlan?.HandledBy}</span><br>` +
      `<span>To: ${
        row.FinalDestinationPoint !== null ? row.FinalDestinationPoint : "N/A"
      }</span><br>`,
    sortable: true,
    firstSortType: "asc",
    html: true, // Important for rendering HTML
...

  {
    ...
    field: (row) => `
    <span class="from-color">Driver: ${
      row.DriverName || "Driver Not Specified"
    }</span><br>
    <span class="to-color">Truck: ${
      row.TruckNumber || "Not Available"
    }</span><br>
    <span class="by-color">By: ${
...
const expecteddispatches = computed(() => {
  const expectedCount = Array.isArray(expectedDispatches)
    ? expectedDispatches.length
    : 0;
...
const getExpectedDispatches = async () => {
  leanseasondispatchStore
    ...
    .then((result) => {
      ...
      expectedDispatches.push(
        ...sorteddata.filter((item) => item.IsIntransit == false)
      );
    })
    .catch((error) => {});
};
