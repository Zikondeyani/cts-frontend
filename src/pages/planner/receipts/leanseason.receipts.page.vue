...
const generateExcel = () => {
  try {
    if (!Array.isArray(receipts) || receipts.length === 0) {
      console.error("❌ No data to export. `receiptsClean` is empty.");
      return;
    }
    ...
    const flattenedData = receipts.flatMap((item) =>
      item.receipts.map((receipt) => ({
        Date: moment(receipt.CreatedOn).format("DD/MM/YYYY"),
        "Delivery Note": receipt.PhysicalDeliveryNote || "N/A",
        District: item.district || "N/A",
        "ATC #": item.atcNumber || "N/A",
        Expected: item.totalExpected || 0,
        Received: item.totalReceived || 0,
        "Received By": item.recipient || "N/A",
      }))
    );
    ...
    console.log("✅ Data to export:", flattenedData);
