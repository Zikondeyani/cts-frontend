              ...
              <form class="bg-white px-4 pb-4 sm:p-6 sm:pb-4">
                ...
                <div class="flex gap-8">
                  ...
                  <div class="flex-1 bg-white rounded-table">
                    ...
                    <p class="mb-4">
                      <strong>Purpose:</strong> {{ instruction.Purpose }}
                    </p>
                    <p class="mb-4">
                      <strong>Vehicle Reg #:</strong>
                      {{ instruction.VehicleRegNo }}
                    </p>
                    <p class="mb-4">
                      <strong>Driver Name:</strong> {{ instruction.DriverName }}
                    </p>
                    <p class="mb-4">
                      <strong>Warehouse (From):</strong>
                      {{ instruction.warehouse.Name }}
                    </p>

                    <!-- ✅ Conditional block -->
                    <template
                      v-if="
                        instruction.activity?.Name === 'Partner Commodity Loan'
                      "
                    >
                      <p class="mb-4">
                        <strong>Activity:</strong>
                        {{ instruction.activity.Name }}
                      </p>
                    </template>
                    <template v-else>
                      <p class="mb-4">
                        <strong>District (To):</strong>
                        {{ instruction.district.Name }}
                      </p>
                      <p class="mb-4">
                        <strong>Transporter:</strong>
                        {{ instruction.transporter.Name }}
                      </p>
                    </template>
                    <!-- ✅ End conditional block -->

                    <p class="mb-4" v-if="instruction.IsRejected !== null">
                      <strong>Comments (If Rejected):</strong>
                      {{ instruction.RejectionComment }}
                    </p>
                    ...
                    <div>
                      <table
                        class="min-w-full bg-white border border-0 rounded-lg"
                      >
                        ...
                        <tbody class="divide-y divide-gray-200">
                          ...
                          >
                            ...
                            <td class="py-2 px-4 border-b">
                              {{ item.Quantity }}
                              {{
                                item.commodity.Unite == "Kg" ? "MT" : "Units"
                              }}
                              ({{ item.NoBags }}
                ...
                <div class="bg-gray-50 px-4 py-3 sm:flex sm:flex-row-reverse">
                  ...
                  <button
                    type="button"
                    v-if="!isRejecting"
                    @click="gotoEditInstruction(instruction.id)"
