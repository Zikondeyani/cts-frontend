        ...
        >
          ...
          >
            ...
            >
              ...
              <form
                @submit="onSubmit"
                :validation-schema="CreateRequisitionSchema"
              >
                  ...
                  <div class="grid grid-cols-6 gap-2">
                    ...
                    <div class="col-span-12 sm:col-span-12">
                      <label
                        for="batch"
                        class="block text-sm font-medium text-gray-700"
                        >Affected Households</label
                      >
                      <input
                        type="number"
                        v-model="AffectedHouseholds"
                        Name="AffectedHouseholds"
                        id="AffectedHouseholds"
                        autocomplete="off"
                        placeholder="Affected Households"
                        class="mt-1 focus:ring-gray-500 focus:border-blue-300 block w-full shadow-sm sm:text-sm border-gray-400 rounded-md"
                      />
                      <p class="text-red-500 text-xs italic pt-1">
                    ...
                    <div class="col-span-12 sm:col-span-12">
                      ...
                      <!-- Tags display -->
                      <div
                        class="flex flex-wrap items-center border-gray-400 rounded-md border p-2 mt-1"
                      >
                        <span
                          v-for="(place, index) in AffectedAreas"
                          :key="index"
                    ...
                    <div class="col-span-12 sm:col-span-12">
                      ...
                      >
                        ...
                        <input
                          type="number"
                          v-model="newGvh"
                          placeholder="Enter Number Affected"
                          class="flex-grow focus:ring-gray-500 focus:border-blue-300 border-none shadow-sm sm:text-sm"
                        />
                      </div>
                    </div>
                    ...
                    <div class="col-span-12 sm:col-span-12">
                      ...
                      <div class="space-y-3">
                        <div
                          v-for="(item, index) in reliefItems"
                          :key="index"
                        ...
                        >
                          <div class="flex-1">
                            ...
                            <select
                              ...
                              @change="validateCommodity(index)"
                          ...
                          >
                            &times;
                            <!-- A simple cross to remove the item -->
                          </button>
                        </div>
                        <button
                          type="button"
                ...
                <div class="px-6 py-4 border-t border-gray-200 bg-gray-50">
                  <div class="flex justify-end space-x-3">
                    ...
                    <button
                      type="submit"
                      class="inline-flex items-center px-3 py-2 text-sm font-medium text-white bg-[#096eb4] hover:text-green-900 rounded-md border border-gray-400 hover:bg-gray-100"
                    >
                      Submit for action
                    </button>
                  </div>
...
const { value: districtId, errorMessage: districtError } =
  useField("districtId");
const { value: AffectedHouseholds, errorMessage: AffectedHouseholdsError } =
  useField("AffectedHouseholds");
  ...
  disasterStore
    ...
    .then((result) => {
      ...
      disasters.push(
        ...result.filter(
          (item) =>
            item.district == user.value.district ||
            item.Requester?.district?.Name == "National"
        )
      );
    })
    .catch((error) => {})
...
const reliefItems = ref([
  { id: 1, commodityId: "", Quantity: "", NoBags: "", error: "" },
]);
...
function validateCommodity(index) {
  ...
  const isDuplicate = reliefItems.value.some(
    (item, idx) => item.commodityId === selectedCommodity && idx !== index
  );
  reliefItems.value[index].error = isDuplicate
    ? "Commodity already added. Please select another."
    : "";
}

function addNewItem() {
  reliefItems.value.push({
    id: reliefItems.value.length + 1,
    commodityId: "",
    Quantity: "",
    NoBags: "",
    error: "",
