    ...
    <div class="bg-white shadow px-4 py-5 sm:rounded-lg sm:p-6">
      <div class="md:grid md:grid-cols-3 md:gap-6">
        <div class="md:col-span-1">
          <h3 class="text-lg font-medium leading-6 text-gray-900">Requisition supporting documents</h3>
...
const onSubmit = useSubmitForm((values, actions) => {
  let newValues = {
    ...
    type: "REQ-DOCUMENT",
    name: "REQUISITION-DODMA-" + model.value.id,
    metadata: {
      publicationDate: moment(model.value.publicationDate).format(
        moment.HTML5_FMT.DATE
      ),
...
const getFiles = async () => {
  ...
  fileStore
    .getByReference({ id: model.value.id, type: "REQ-DOCUMENT" })
    .then((result) => {
      files.length = 0;
      files.push(...result);
    })
...
const create = async (data) => {
  ...
  fileStore
    ...
    .then((result) => {
      Swal.fire({
        title: "Success",
        text: "Requisition Attachments Added!",
        icon: "success",
        confirmButtonText: "Ok",
      });
    })
