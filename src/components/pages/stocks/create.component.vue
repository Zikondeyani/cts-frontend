                ...
                <div class="px-4 py-5 bg-white sm:p-6">
                  <div class="grid grid-cols-6 gap-2">
                    ...
                    <div class="col-span-6 sm:col-span-3">
                      ...
                      <select id="stock-type" name="type" v-model="stockType"
                        class="mt-1 focus:ring-gray-500 focus:border-blue-300 block w-full shadow-sm sm:text-sm border-gray-400 rounded-md">
                    ...
                    <div class="col-span-6 sm:col-span-3">
                      ...
                      <input type="text" id="batch-number" v-model="BatchNumber" placeholder="Enter batch number"
                        class="mt-1 focus:ring-gray-500 focus:border-blue-300 block w-full shadow-sm sm:text-sm border-gray-400 rounded-md" />
                      <p class="text-red-500 text-xs italic pt-1">
                        {{ BatchNumberError }}
                      </p>
                    ...
                    <div class="col-span-6 sm:col-span-3">
                      ...
                      <input type="text" id="from-who-search" v-model="fromWhoInput" placeholder="Type or select source"
                        @input="filterFromWhoOptions"
                        class="mt-1 focus:ring-gray-500 focus:border-blue-300 block w-full shadow-sm sm:text-sm border-gray-300 rounded-md" />
                      <ul v-if="filteredFromWho.length"
                        class="border border-gray-300 rounded-md mt-2 max-h-48 overflow-y-auto bg-white z-10 relative">
                        <li v-for="(source, index) in filteredFromWho" :key="index" @click="selectFromWho(source)"
...
const getWarehouses = async () => {
  warehouseStore
    ...
    .then((result) => {
      ...
      warehouses.push(
        ...result.filter((item) => {
          const isDodma = item.organisation?.Name === "DODMA";

          const matchesScope = isNational
            ? true
            : item.district?.Name === user.value.district;

          return isDodma && matchesScope;
        })
      );
    })
    .catch((error) => { })
