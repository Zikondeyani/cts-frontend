  ...
  <div class="min-h-full font-body" style="background-color: #096eb4">
    ...
    <div class="max-w-7xl mx-auto flex justify-between items-center flex-wrap lg:flex-nowrap">
        ...
        <div class="flex items-center justify-between w-full lg:w-auto">
          ...
          <span class="font-bold text-white mx-4 hidden lg:block">DODMA CTS | Auditor
        ...
        <!-- Mobile Admin Text -->
        <span class="font-bold text-white mx-4 block lg:hidden">DODMA CTS | Auditor
          <span class="text-xs font-normal">(v{{ versiondata.version }})</span>
        </span>
          ...
          <Menu as="div" class="flex-shrink-0 relative">
            ...
            <transition enter-active-class="transition ease-out duration-100" enter-from-class="transform opacity-0 scale-95" enter-to-class="transform opacity-100 scale-100" leave-active-class="transition ease-in duration-75" leave-from-class="transform opacity-100 scale-100" leave-to-class="transform opacity-0 scale-95">
              <MenuItems class="origin-top-right absolute z-10 right-0 mt-2 w-48 rounded-md shadow-lg bg-white ring-1 ring-black ring-opacity-5 py-1 focus:outline-none">
                ...
                <MenuItem v-slot="{ active }">
                <router-link to="/auditor/change-password" :class="menuItemClasses(active, true)">
...
const onAbout = async () => {
   $router.push({ path: "/auditor/about-system" })
...
function gotoSystemsettings() {
  $router.push({ path: '/auditor/system' });
...
function navigation() {
  let navList = [
    { name: "Home", href: "/auditor/dashboard", icon: HomeIcon, current: false },
   

    { name: "Reports", href: "/auditor/report-management", icon: DocumentTextIcon, current: false },
  ...
  navList.forEach(navItem => {
    ...
    const isMatched = currentRouteBase === navItem.href ||
      (navItem.name === "Plan & Dispatch" && (currentRouteBase.startsWith("/auditor/loadingplans") || currentRouteBase.startsWith("/auditor/dispatches"))) ||
      (navItem.name === "Project Management" && (currentRouteBase.startsWith("/auditor/dispatch-management") || currentRouteBase.startsWith("/auditor/loadingplans") || currentRouteBase.startsWith("/auditor/dispatches") ))||
      (navItem.name === "Receipts" && (currentRouteBase.startsWith("/auditor/receipts")));
