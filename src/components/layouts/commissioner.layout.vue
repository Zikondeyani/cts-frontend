          ...
          <Menu as="div" class="flex-shrink-0 relative">
            ...
            >
              ...
              >
                ...
                <MenuItem v-slot="{ active }">
                  <router-link
                    to="/commissioner/change-password"
                    :class="menuItemClasses(active, true)"
                  >
...
const navItems = computed(() => {
  const currentRouteBase = useRouter()
    .currentRoute.value.fullPath.split("/")
    .slice(0, 3)
    .join("/");
  return navigation().map((item) => ({
    ...item,
    current: currentRouteBase === item.href,
  }));
});
...
const getInstructions = async () => {
  ...
  instructionsStore
    ...
    .then((result) => {
      ...
      instructions.push(
        ...result.filter(
          (item) => !item.IsApproved && item.instructedCommodities
        )
      );
