<template>
  <div class="min-h-full font-body" style="background-color: #096eb4">
    ...
    <!-- Feedback Modal -->
    <div v-if="isFeedbackOpen" class="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
...
const showFeedbackModal = ref(false); // Controls the visibility of the feedback modal
const feedbackText = ref(""); // Holds the feedback text entered by the user
...
const submitFeedback = () => {
  if (feedbackText.value.trim()) {
    ...
    let model = {
      user: user.value.firstname + " " + user.value.lastname,
    ...
    feedbackstore
      ...
      .then((result) => {
        Swal.fire({
          text: "Successfully captured feedback",
          icon: "success",
          toast: true,
          position: "top-right",
    ...
    isFeedbackOpen.value = false; // Close the modal
  } else {
    Swal.fire({
...
const getExpectedDispatches = async () => {
  try {
    ...
    const filteredDispatches = reversedData.filter(
      (item) =>
        !item.IsArchived &&
        item.instruction.district?.Name === user.value.district
    );
...
function navigation() {
  let navList = [
    {
      name: "Home",
      href: "/receipient/dashboard",
      icon: HomeIcon,
      current: false,
    },
    {
      name: "Requisitions",
      href: "/receipient/requisition-management",
   ...
   */ /*  { name: "Dispatches", href: "/receipient/dispatch-management", icon: AdjustmentsIcon, current: false },
    ...
    {
      name: "Disasters",
      href: "/receipient/emergency-management",
    ...
    {
      name: "Dispatches",
      href: "/receipient/project-management",
