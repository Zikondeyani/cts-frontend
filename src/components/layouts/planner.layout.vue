          ...
          <Menu as="div" class="flex-shrink-0 relative">
            ...
            <transition enter-active-class="transition ease-out duration-100"
              ...
              <MenuItems
                ...
                <MenuItem v-slot="{ active }">
                <router-link to="/planner/change-password" :class="menuItemClasses(active, true)">
...
function navigation() {
  let navList = [
    ...
    { name: "Requisitions", href: "/planner/requisition-management", icon: ClipboardListIcon, current: false },
    { name: "Instructions", href: "/planner/instruction-management", icon: CollectionIcon, current: false },
    { name: "Reports", href: "/planner/report-management", icon: DocumentTextIcon, current: false },
  ];
  ...
  navList.forEach(navItem => {
    const isMatched = currentRouteBase === navItem.href ||
      (navItem.name === "Project Management" && (currentRouteBase.startsWith("/planner/dispatch-management") || currentRouteBase.startsWith("/planner/loadingplans") || currentRouteBase.startsWith("/planner/dispatches"))) ||
...
const updateNotifications = () => {
  ...
  if (newRequisitionsCount.value > 0) {
    notifications.value.push({
      ...
      href: "/planner/requisition-management"
    });
  }
...
const getRequisitions = async () => {
  ...
  requisitionsStore
    ...
    .then((result) => {
      ...
      requisitions.push(
        ...result.filter(
          item => (item.IsArchived === false || item.IsArchived === null) && item.status !== 3
        )
      );
